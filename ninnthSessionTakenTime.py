...
def parse_appointments(data):
    ...
    # --- NEW: parse taken_time_in_week and store in a list ---
    taken_slots = []
    if "taken_time_in_week" in data:
        for item in data["taken_time_in_week"]:
            for day_info in item["days"]:
                day_name = day_info["day"]
                if day_name in weekday_names:
                    ...
                    for tf in day_info["time_frames"]:
                        ...
                        taken_slots.append((day_index, start, end))

    # Return both appointments and the taken_slots
    return appointments, taken_slots
...
def schedule_appointments(appointments, settings, taken_slots):
    # Create the blank calendar
    calendar = initialize_calendar(settings)

    # Mark 'taken' slots as already occupied
    for (day_index, start, end) in taken_slots:
        for slot in calendar[day_index]:
            if slot.start_time >= start and slot.start_time < end:
                # Any non-None value means the slot is occupied
                slot.client_id = "TAKEN"

    used_field_hours = [0]*6

    # Now run the backtracking scheduler on the updated calendar
...
def schedule_endpoint():
    ...
    # Parse appointments AND taken slots
    appointments, taken_slots = parse_appointments(data)

    # Perform scheduling with the pre-marked slots
    success, final_schedule, unscheduled_tasks = schedule_appointments(appointments, settings, taken_slots)
